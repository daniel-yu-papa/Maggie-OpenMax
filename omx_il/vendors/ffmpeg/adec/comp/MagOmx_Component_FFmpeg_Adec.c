    ...
    do {
        ...
        if (!got_frame){
            ...
            */
            ...
            COMP_LOGV(root, "Succeed to decode the audio frame (len: %d, pts: 0x%x)! Send to output buffer: %p[%p]", 
                             decodedLen, decodedFrame->pts, destbufHeader, destbufHeader->pBuffer);
