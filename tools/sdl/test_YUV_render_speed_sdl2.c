/*test the YUV picture rendering speed in SDL2*/
#include <stdlib.h>
#include <stdio.h>
...
void UpdateTexture(SDL_Texture *texture, Uint8 *imageYUV, Uint32 width, Uint32 height) 
{ 
    ...
    if(texture && !SDL_LockTexture(texture, NULL, (void **)&pixels, &pitch) ) { 
        ...
        if(pitch == width) { 
            ...
            memcpy(pixels, imageYUV, size * 3 /2); 
        } 
        else { 
            register unsigned char *y1,*y2,*y3,*i1,*i2,*i3; 
            int i; 
            y1 = pixels; 
            y3 = pixels + pitch * height; // invertiti xche' avevo i colori sballati! 
            y2 = pixels + pitch * height * 5 / 4; 


            i1=imageYUV; 
            i2=imageYUV + width * height; 
            i3=imageYUV + width * height * 5 / 4; 
...
main(int argc, char **argv)
{
    ...
    while (!done) {
        ...
        UpdateTexture(texture, imageYUV, image->w, image->h);
