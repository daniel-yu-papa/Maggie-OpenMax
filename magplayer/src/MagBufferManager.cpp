...
#undef MODULE_TAG
#endif          
#define MODULE_TAG "MMP_Buffer"

MagBufferManager::MagBufferManager(ui32 size, ui32 num, bool block):
                                    mBufSize(size),
                                    mBufNum(num),
                                    mFreeNodeNum(num),
                                    mBlock(block),
...
MagBufferManager::~MagBufferManager(){
    List_t *next;
 ...
 */
OMX_ERRORTYPE MagBufferManager::create(OMX_HANDLETYPE hComp, ui32 portIdx, OMX_HANDLETYPE privData){
    ui32 i = 0;
    OMX_ERRORTYPE err;
    Omxil_BufferNode_t *bufNode;
...
OMX_BUFFERHEADERTYPE *MagBufferManager::get(){
    List_t *next = NULL;
...
void MagBufferManager::put(OMX_BUFFERHEADERTYPE *bufHeader){
    List_t *next;
...
bool MagBufferManager::needPushBuffers(void){
    if (mFreeNodeNum > 1){
        return true; 
    }else{
...
void MagBufferManager::start(){
    mStopped = false;
}

void MagBufferManager::stop(){
    mStopped = true;
    Mag_SetEvent(mPutBufEvent);
}