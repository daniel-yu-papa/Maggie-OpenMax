...
static magMempoolInternal_t *createMemPoolInter(magMempoolHandle hMemPool, unsigned int bytes){
    ...
    pInterMP = (magMempoolInternal_t *)mag_malloc(sizeof(magMempoolInternal_t));
    if (NULL == pInterMP){
        return NULL;
    }
    ...
    list_add_tail(&pInterMP->node, &hMemPool->memPoolListHead);
    ...
    if ((mb->end - mb->start) > bytes){
        ...
        *getBuffer = mb->pBuf;
        ...
        if(mb->end == mpInter->memPoolSize){
            ...
            AGILE_LOGV("MB[s:%d - e:%d] is not the nearest end MB", mb->start, mb->end);
        }
    }

    if (NULL == *getBuffer){
        if (mb2)
            AGILE_LOGD("MB1(s:%d - e:%d) or MB2(s:%d - e:%d) do not fit for required size(%d bytes)", mb->start, mb->end, 
                        mb2->start, mb2->end, bytes);
        else
            AGILE_LOGD("MB1(s:%d - e:%d) does not fit for required size(%d bytes)", mb->start, mb->end, bytes);
    }
    return MAG_ErrNone;
...
magMempoolHandle magMemPoolCreate(unsigned int bytes){
    ...
    hMP = (magMempoolHandle)mag_malloc(sizeof(magMemPoolMgr_t));
    if (NULL == hMP){
        return NULL;
    }  
...
MagErr_t magMemPoolPutBuffer(magMempoolHandle hMemPool, void *pBuf){
    ...
    if (NULL != mb){
        ...
        if (lsecond != &mpool->freeMBListHead){
            if (secondMB->end == firstMB->start){
                AGILE_LOGD("merge the adjacent MB[1]: 1st[s:%d - e:%d] and 2nd[s:%d - e:%d]", firstMB->start, firstMB->end, 
                            secondMB->start, secondMB->end);
                firstMB->start = secondMB->start;
                list_del(lsecond);
                list_add(lsecond, &hMemPool->unusedMBListHead);  
            }else if (secondMB->start == firstMB->end){
                AGILE_LOGD("merge the adjacent MB[2]: 1st[s:%d - e:%d] and 2nd[s:%d - e:%d]", firstMB->start, firstMB->end, 
                            secondMB->start, secondMB->end);
